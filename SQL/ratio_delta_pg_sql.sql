...
RETURN CASE
  ...
  WHEN round_to IS NOT NULL
   THEN CAST(
     round(
       CAST(scaled_fused_divide_increment(a, b, scale) AS numeric),
       round_to
     )
     AS double precision
   )
...
RETURN CASE
  ...
  WHEN round_to IS NOT NULL
   THEN CAST(
     round(
       CAST(scaled_fused_divide_decrement(a, b, scale) AS numeric),
       round_to
     )
     AS double precision
   )
...
RETURN CASE
  ...
  WHEN is_absolute AND round_to IS NOT NULL
   THEN CAST(
     round(
       CAST(
         scaled_fused_absolute_divide_add(a, b, c, scale) AS numeric
       ),
       round_to
     )
     AS double precision
...
RETURN CASE
  ...
  WHEN round_to IS NOT NULL
   THEN CAST(
     round(
       CAST(scaled_fused_divide_subtract(a, b, c, scale) AS numeric),
       round_to
     )
     AS double precision
   )
